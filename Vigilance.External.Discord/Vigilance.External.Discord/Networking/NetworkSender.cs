...
namespace Vigilance.API.Discord.Networking
{
    public abstract class NetworkSender : IDisposable
    {
        ...
        public NetworkSender(TimeSpan reconnectionInterval)
            : this(null, reconnectionInterval)
        {
        }

        public NetworkSender(string ipAddress, ushort port)
            : this(new IPEndPoint(IPAddress.Parse(ipAddress), port))
        {
        }

        public NetworkSender(string ipAddress, ushort port, TimeSpan reconnectionInterval)
            : this(new IPEndPoint(IPAddress.Parse(ipAddress), port), reconnectionInterval)
        {
        }

        public NetworkSender(IPEndPoint ipEndPoint)
            : this(ipEndPoint, TimeSpan.FromSeconds(5))
        {
        }

        public NetworkSender(IPEndPoint ipEndPoint, TimeSpan reconnectionInterval)
        {
            IPEndPoint = ipEndPoint;
            ReconnectionInterval = reconnectionInterval;
        }

        ~NetworkSender() => Dispose(false);
