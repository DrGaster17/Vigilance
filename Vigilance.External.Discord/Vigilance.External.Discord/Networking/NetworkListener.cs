...
namespace Vigilance.API.Discord.Networking
{
    public abstract class NetworkListener : IDisposable
    {
        ...
        public NetworkListener(TimeSpan reconnectionInterval)
            : this(null, reconnectionInterval)
        {
        }

        public NetworkListener(string ipAddress, ushort port)
            : this(new IPEndPoint(IPAddress.Parse(ipAddress), port))
        {
        }

        public NetworkListener(string ipAddress, ushort port, TimeSpan reconnectionInterval)
            : this(new IPEndPoint(IPAddress.Parse(ipAddress), port), reconnectionInterval)
        {
        }

        public NetworkListener(IPEndPoint ipEndPoint)
            : this(ipEndPoint, TimeSpan.FromSeconds(5))
        {
        }

        public NetworkListener(IPEndPoint ipEndPoint, TimeSpan reconnectionInterval)
        {
            IPEndPoint = ipEndPoint;
            ReconnectionInterval = reconnectionInterval;
        }

        ~NetworkListener() => Dispose(false);
