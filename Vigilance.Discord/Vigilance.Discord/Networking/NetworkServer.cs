...
namespace Vigilance.Discord.Networking
{
    public abstract class NetworkServer : IDisposable
    {
        ...
        public NetworkServer(TimeSpan reconnectionInterval)
            : this(null, reconnectionInterval)
        {
        }

        public NetworkServer(string ipAddress, ushort port)
            : this(new IPEndPoint(IPAddress.Parse(ipAddress), port))
        {
        }

        public NetworkServer(string ipAddress, ushort port, TimeSpan reconnectionInterval)
        ...
        public NetworkServer(IPEndPoint ipEndPoint, TimeSpan reconnectionInterval)
        {
            IPEndPoint = ipEndPoint;
            ReconnectionInterval = reconnectionInterval;
        }

        ~NetworkServer() => Dispose(false);
