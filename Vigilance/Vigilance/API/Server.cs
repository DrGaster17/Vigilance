...
namespace Vigilance.API
{
    public static class Server
    {
        ...
        public static string Pastebin { get => CompCache.CharacterClassManager == null ? "" : CompCache.CharacterClassManager.NetworkPastebin; }
        public static string Password { get => ServerConsole.Password; set => ServerConsole.Password = value; }
        public static string ServerRandom { get => CompCache.CharacterClassManager == null ? "" : CompCache.CharacterClassManager._hub.queryProcessor.NetworkServerRandom; }
        public static string IpAddress { get => ServerConsole.Ip; }
        public static string Version => GameCore.Version.VersionString;

        public static bool Ban(Player player, int duration = 1, string message = "You have been banned.", string issuer = "Server", string reason = "No reason given.") => CompCache.BanPlayer.BanUser(player.GameObject, duration, reason, issuer);
        ...
        public static void RunCommand(string command) => CommandProcessor.ProcessQuery(command, CompCache.ReferenceHub.queryProcessor._sender);
        ...
        public static void Restart(bool safeRestart = true)
        {
            if (safeRestart)
            {
                CompCache.PlayerStats.Roundrestart();
