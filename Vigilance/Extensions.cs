...
namespace Vigilance.Extensions.Rpc
{
	public static class RpcExtensions
    {
		...
		public static void RpcBlood(this Player player, Vector3 pos, int type, float f)
        {
			...
			player.Hub.characterClassManager.SendRPCInternal(typeof(CharacterClassManager), "RpcPlaceBlood", writer, 0);
			NetworkWriterPool.Recycle(writer);
		}

		public static void RpcRoundStart(this Player player)
        {
			NetworkWriter writer = NetworkWriterPool.GetWriter();
			player.Hub.characterClassManager.SendRPCInternal(typeof(CharacterClassManager), "RpcRoundStarted", writer, 0);
			NetworkWriterPool.Recycle(writer);
		}
        ...
        {
			NetworkWriter writer = NetworkWriterPool.GetWriter();
			writer.WriteBoolean(isClassD);
			writer.WriteBoolean(changeTeam);
			Map.Find<Escape>()?.SendTargetRPCInternal(player.Connection, typeof(Escape), "TargetShowEscapeMessage", writer, 0);
			NetworkWriterPool.Recycle(writer);
		}
