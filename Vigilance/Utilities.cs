...
namespace Vigilance.Utilities
{
    public static class Utils
    {
        ...
        {
            if (!PluginManager.CompatibleVersions.Contains(Server.Version))
            {
                Log.Add("PluginManager", $"This version ({PluginManager.Version}) is not compatible with your server version ({GameCore.Version.VersionString})!\nCompatible versions: {PluginManager.CompatibleVersions.AsString()}", LogType.Error);
                return;
            }

            if (PluginManager.Version.IsTesting) Log.Add("PluginManager", "This is a development version in testing. Except to see some bugs.", LogType.Warn);
            if (PluginManager.Version.IsBeta) Log.Add("PluginManager", "This is a beta version. Except to see some bugs.", LogType.Warn);
            Paths.CheckMainConfig();
            PluginManager.Config = new YamlConfig(Paths.ConfigPath);
            CommandManager.Enable();
            Paths.CheckDirectories();
            Paths.CheckDependencies();
            PluginManager.Assemblies = new Dictionary<string, Assembly>();
            PluginManager.Plugins = new Dictionary<string, Plugin>();
            PluginManager.Dependencies = new Dictionary<string, Assembly>();
        ...
        public static ItemType[] UpgradeItemIds(ItemType input)
        {
            ...
            ItemType[] result;
            if (!Map.Scp914.Singleton.recipesDict.TryGetValue(input, out dictionary) || !dictionary.TryGetValue(Map.Scp914.Singleton.knobState, out result)) return null;
            return result;
        }
        ...
        {
            if (!RagdollManager_SpawnRagdoll.Ragdolls.ContainsKey(p)) RagdollManager_SpawnRagdoll.Ragdolls.Add(p, new List<Ragdoll>());
            return RagdollManager_SpawnRagdoll.Ragdolls[p];
        }

        public static List<Pickup> GetPickups(Player p)
        {
            if (!Inventory_CallCmdDropItem.Pickups.ContainsKey(p)) Inventory_CallCmdDropItem.Pickups.Add(p, new List<Pickup>());
            return Inventory_CallCmdDropItem.Pickups[p];
        }
        ...
        {
            if (value) player.Hub.characterClassManager.CmdRequestHideTag(); else player.Hub.characterClassManager.CallCmdRequestShowTag(false);
        }

        public static ulong GetParsedId(Player player)
        {
            string id = string.IsNullOrEmpty(player.UserId) ? "Dedicated Server" : player.UserId.Split('@')[0];
            if (id == "Dedicated Server") return 0;
            if (!ulong.TryParse(id, out ulong user)) return 0;
            return user;
        }

        public static UserIdType GetIdType(Player player)
        {
            if (string.IsNullOrEmpty(player.UserId)) return UserIdType.LocalPlayer;
            if (player.UserId.Contains("@steam")) return UserIdType.Steam;
            if (player.UserId.Contains("@discord")) return UserIdType.Discord;
            if (player.UserId.Contains("@northwood")) return UserIdType.Northwood;
            if (player.UserId.Contains("@patreon")) return UserIdType.Patreon;
            return UserIdType.Unspecified;
        }

        public static Room GetRoom(Player player)
        {
            Vector3 end = player.Position - new Vector3(0f, 10f, 0f);
            bool flag = Physics.Linecast(player.Position, end, out RaycastHit raycastHit, -84058629);
            if (!flag || raycastHit.transform == null) return null;
            Transform latestParent = raycastHit.transform;
            while (latestParent.parent?.parent != null) latestParent = latestParent.parent;
            foreach (Room room in Map.Rooms) if (room.Transform == latestParent) return room;
        ...
        public static void SetScale(Player player, Vector3 scale)
        {
            try
            {
                ...
                foreach (GameObject obj in PlayerManager.players)
                {
                    ...
                    if (obj != player.GameObject) playerCon.Send(destroyMessage, 0);
        ...
        {
            if (player.Hub.inventory.items.Count >= 8) Map.SpawnItem(item, player.Position, player.Rotations); else player.Hub.inventory.AddNewItem(item);
        }
    ...
    public static class Handling
    {
        ...
        public static void OnConsoleCommand(string cmd, Player ply, bool all, out bool allow, out string reply, out string color)
        {
            try
            {
                ...
                allow = CommandManager.CallCommand(ply, args, out reply, out color);
        ...
        public static void OnPlayerJoin(Player ply)
        {
            try
            {
                ...
                if (!ply.Session.ContainsKey("joinDate")) ply.Session.Add("joinDate", DateTime.UtcNow); else ply.Session["joinDate"] = DateTime.UtcNow;
            }
            catch (Exception e)
            {
                Log.Add("Handling", e);
        ...
        public static void OnScp914UpgradePickup(Pickup input, out ItemType output, out bool allow)
        {
            try
            {
                ...
                ev.Output = Utilities.Utils.UpgradeItemId(input.ItemId);
            ...
            {
                Log.Add(nameof(Handling.OnScp914UpgradePickup), e);
                output = Utilities.Utils.UpgradeItemId(input.ItemId);
                allow = true;
            }
        }
        ...
        public static void OnScp914UpgradeItem(ItemType input, bool all, out ItemType output, out bool allow)
        {
            try
            {
                ...
                ev.Output = Utilities.Utils.UpgradeItemId(input);
                EventManager.Trigger<Scp914UpgradeItemHandler>(ev);
                output = ev.Output;
                allow = ev.Allow;
            }
            ...
            {
                Log.Add(nameof(Handling.OnScp914UpgradeItem), e);
                output = Utilities.Utils.UpgradeItemId(input);
                allow = true;
            }
        }
        ...
        public static void OnScp914UpgradePlayer(Player player, bool all, out bool allow)
        {
            ...
            {
                Log.Add(nameof(Handling.OnScp914UpgradePlayer), e);
                allow = true;
            }
        }
        ...
        public static void OnUseLocker(Player player, Locker locker, string token, bool all, out string accessToken, out bool allow)
        {
            ...
            {
                Log.Add("Handling.OnUseLocker", e);
                accessToken = token;
                allow = all;
            }
        }
        ...
        public static void OnSwitchLever(Player player, bool c, bool st, bool all, out bool state, out bool allow)
        {
            ...
            {
                Log.Add("Handling.OnSwitchLever", e);
                state = st;
                allow = all;
            }
        }
        ...
        public static void OnGenerateSeed(int seed, out int newSeed)
        {
            ...
            {
                Log.Add("Handling.OnGenerateSeed", e);
                newSeed = seed;
            }
        }
