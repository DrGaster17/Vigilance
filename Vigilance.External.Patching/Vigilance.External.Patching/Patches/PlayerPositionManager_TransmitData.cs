...
namespace Vigilance.External.Patching.Patches
{
    ...
    {
        public static readonly Vector3 GhostPos = Vector3.up * 6000f;
        ...
        public static bool Prefix(PlayerPositionManager __instance)
        {
            try
            {
                ...
                List<GameObject> players = ListPool<GameObject>.Shared.Rent(PlayerManager.players);
                ...
                foreach (GameObject gameObject in players)
                {
                    ...
                    if (player.Role.Is939())
                    {
                        for (int index = 0; index < __instance._usedData; ++index)
                        {
                            if (__instance._transmitBuffer[index].position.y < 800f)
                            {
                                ...
                                if (hub2.characterClassManager.CurRole.team != Team.SCP && hub2.characterClassManager.CurRole.team != Team.RIP
                                    && !hub2.GetComponent<Scp939_VisionController>().CanSee(player.ReferenceHub.characterClassManager.Scp939))
                                {
                    ...
                    for (var z = 0; z < __instance._usedData; z++)
                    {
                        ...
                        if (target.IsInvisible || PlayerCannotSee(player, target))
                        {
                            MakeGhost(z, __instance._transmitBuffer);
                        }
                        else if (player.Role == RoleType.Scp173 && ((!PluginManager.Config.CanTutorialBlockScp173 && target.Role == RoleType.Tutorial)))
                        {
            ...
            {
                Patcher.Log(typeof(PlayerPositionManager_TransmitData), e);
                return true;
            }
        }

        private static Vector3 FindLookRotation(Vector3 player, Vector3 target) => (target - player).normalized;
        private static bool PlayerCannotSee(Player source, Player target) => source.TargetGhosts.Contains(target) || source.TargetGhosts.Contains(target);
        ...
        private static void RotatePlayer(int index, PlayerPositionData[] buff, Vector3 rotation) => buff[index] = new PlayerPositionData(buff[index].position, Quaternion.LookRotation(rotation).eulerAngles.y, buff[index].playerID);

        private static Player GetPlayerOrServer(GameObject gameObject)
        {
            var refHub = ReferenceHub.GetHub(gameObject);
            return refHub.isLocalPlayer ? LocalComponents.Player : PlayersList.GetPlayer(gameObject);
        }
    }
}