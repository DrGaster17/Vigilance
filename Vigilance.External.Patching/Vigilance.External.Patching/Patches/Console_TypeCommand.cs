    ...
    public static class Console_TypeCommand
    {
        public static bool Prefix(Console __instance, string cmd, CommandSender sender = null)
        {
            try
            {
                ...
                if (CommandManager.Run(sender == null || sender.SenderId == "SERVER CONSOLE" ? LocalComponents.Player : PlayersList.GetPlayer(sender.SenderId), cmd.Replace("@", "").Replace("/", "").Replace(".", ""), 
