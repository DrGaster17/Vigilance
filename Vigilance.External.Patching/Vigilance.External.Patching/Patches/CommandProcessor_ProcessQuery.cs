            ...
            try
            {
                ...
                if (!ev.Allow)
                {
                    if (!string.IsNullOrEmpty(ev.Response))
                        ...
                        Log.Add("Vigilance.External.Patching.Patches.CommandProcessor.ProcessQuery", $"The \"{typeof(RemoteAdminCommandEvent).FullName}\" event has been disallowed by a server plugin, but the response is an empty string.", LogType.Warn);
